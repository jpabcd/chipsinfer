...
- 页面右上角可以导出当前筛选条件命中的全部图片路径 JSON，不受当前页码、分页数量、Shuffle 或 JSON 优先级影响
- 页面右上角可以导入别人导出的 JSON，并在同一图片路径下回显评价结果
- 支持在“推理结果 JSON”中手动填写路径，系统会自动从固定本地候选文件读取 JSON 列表并可点击选中
...
- 大型推理 JSON 会在后台建立并复用本地解析缓存；默认每页显示 2 列 × 10 行，以减少首屏图片和交互卡片的加载量
- 图片不会一次性全部解码到内存：服务端只对当前页图片使用 OpenCV 读取尺寸，缓存中仅保存宽高；图片文件通过 HTTP 分块传输，浏览器继续使用懒加载
...
点击“导出筛选图片路径”会下载 `filtered_image_paths.json`。导出会应用当前的推理结果 JSON、工程/数据根目录、光照通道、模型判定、关键词、图片搜索以及混淆矩阵筛选；不会只导出当前页。每条记录包含图片绝对路径、文件名和相对于数据根目录（或工程目录）的来源目录：
...
```json
{
  ...
  "items": [
    {
      "imagePath": "/workspace/data/light_1/yolo_pred_images/sample_NG.jpg",
      "imageName": "sample_NG.jpg",
      "source": "outputs/run_xxx/predict_input/light_1"
    }
  ]
}
```

该文件可直接作为后续训练/复制脚本的图片清单使用。

## 固定候选文件读取推理结果 JSON

系统会自动读取应用目录下的固定文件 `inspection_web_app/json_candidates.txt`，每一行都作为一个候选路径（支持空行和 `#` 注释行），并展示为可点击列表。

- 点击某一条候选后，会自动填入“推理结果 JSON”并立即执行渲染。
- 列表中不存在的 JSON 路径会显示为不可点击。
- 候选相对路径会优先相对于固定 TXT 所在目录或工程目录解析。
- 手动输入过的 JSON 路径会被自动记忆，即使它不在最初的候选 TXT 中，也会继续出现在候选列表里。
